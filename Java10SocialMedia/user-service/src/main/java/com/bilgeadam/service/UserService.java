...
public class UserService extends ServiceManager<UserProfile, Long> {
    ...
    public String updateUserProfile(UserProfileUpdateRequestDto dto) {
        ...
        // cacheManager.getCache("find_by_username").evict(userProfile.get().getUsername()); // Silme İşlemi

        cacheManager.getCache("find_by_username").put(userProfile.get().getUsername(), userProfile.get()); // Güncelleme
